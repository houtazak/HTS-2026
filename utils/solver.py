...
def newton(fes : ngs.FESpace,                                                       # finite element space
    ...
    def compute_res2(sol):
        """Calculate |r(sol)|^2 with the use of multithreading"""
        if use_multithreading:
            with ngs.TaskManager():
                res_vec = ngs.LinearForm(residual(sol, v)).Assemble().vec
        else:
            res_vec = ngs.LinearForm(residual(sol, v)).Assemble().vec
        
        # Extract the free degrees of freedom
        free_dofs = fes.FreeDofs()
        res_np = res_vec.FV().NumPy()
        # Convert BitArray to numpy mask
        free_mask = np.array(free_dofs, dtype=bool)
        return norm(res_np[free_mask])**2
    ...
    while 1:
        ...
        if linesearch :
            ...
            res2_ls = compute_res2(state_linesearch)
